...
export async function fetchPublicLeaderboardSnapshot() {
  ...
  return {
    entries: liveStats.strengthLeaderboard?.rows || [],
    groupEntries: liveStats.calisthenicsLeaderboard?.rows || [],
    streakEntries: liveStats.topStreaks?.rows || [],
    streakLiveMessage: liveStats.topStreaks?.rows?.[0]?.message || "",
    liveEvents: [],
    pebble: {},
    usersToday: liveStats.usersUsingToday || 0,
    usersThisWeek: liveStats.totalUsersThisWeek || 0,
    usersOnline: 0,
    workoutsLogged: liveStats.workoutsLogged || 0,
    caloriesTracked: liveStats.caloriesTracked || 0,
    gallonsDrank: liveStats.gallonsDrank || 0,
    directories: {},
...
export async function fetchLiveMetrics(windowValue = "today") {
  ...
  return {
    requestedWindow: windowValue,
    generatedAt: new Date().toISOString(),
    masterLogSheetUrl: "",
    usersUsingToday: metricRecord(liveStats.usersUsingToday || 0, ""),
    totalUsersThisWeek: metricRecord(liveStats.totalUsersThisWeek || 0, ""),
    usersOnline: metricRecord(0, ""),
    workoutsLogged: metricRecord(liveStats.workoutsLogged || 0, ""),
    caloriesTracked: metricRecord(liveStats.caloriesTracked || 0, ""),
    gallonsDrank: metricRecord(liveStats.gallonsDrank || 0, ""),
  };
}
