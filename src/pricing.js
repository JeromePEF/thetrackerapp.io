...
// ── Viewport fitting (same as homepage) ─────────────────────────────────────

let fitFrame = 0;

function fitPricingToViewport() {
  ...
  const cw = shell.scrollWidth;
  const ch = shell.scrollHeight;

  if (!cw || !ch || !vw || !vh) return;

  const scale = Math.min(vw / cw, vh / ch, 1);
  const ox = Math.max((vw - cw * scale) / 2, 0);
  const oy = Math.max((vh - ch * scale) / 2, 0);
...
function requestFit() {
  ...
  fitFrame = window.requestAnimationFrame(fitPricingToViewport);
}
