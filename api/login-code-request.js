...
const REQUEST_ENDPOINTS = ["/api/auth/login-code/request", "/api/auth/code/request", "/api/login-code/request"];
...
async function tryEndpoints(payload) {
  ...
  return {
    ...
    message: lastFailure?.message || "Unable to request login code.",
...
export default async function handler(req, res) {
  ...
  if (!["email", "phone", "username"].includes(method)) {
    return res.status(400).json({
      ok: false,
